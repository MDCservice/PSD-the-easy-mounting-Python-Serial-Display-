...
type
  ...
  TlvLine = class(TShape)

  end;

  TlvLMeter = class(TplA3nalogGauge)

  end;

  TlvMsgBox = class(TSpeedbutton)
  public
    constructor Create(AOwner: TComponent); override;
  end;

  TlvObjMask = class(TSpeedbutton)
  public
    constructor Create(AOwner: TComponent); override;
  end;

  TlvPage = class(TScrolbox);
...
begin
     ...
     try
       ...
       case sClassName of
         ...
         'TlvLMeter': retPInt := TlvLMeter.Create( TControl(aOwner)  );
         'TlvMsgBox': retPInt := TlvMsgBox.Create( TControl(aOwner)  );
         'TlvObjMask': retPInt := TlvObjMask.Create( TControl(aOwner)  );
         'TlvScrollbox': retPInt := TlvMsgBox.Create( TControl(aOwner)  );
         else retPInt := nil;
       end;
...
constructor TlvMsgBox.Create(AOwner: TComponent);
begin
     inherited create(aOwner);
...
constructor TlvObjMask.Create(AOwner: TComponent);
begin
      inherited create(aOwner);
      self.Parent := TWinControl(aOwner);
      self.Width := 62;
      self.Caption := 'ObjMask';
end;
...
initialization
  ...
  RegisterClass(TlvLMeter);
  RegisterClass(TlvMsgBox);
  RegisterClass(TlvObjMask);
  RegisterClass(TlvPage);
